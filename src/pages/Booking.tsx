...
export default function Booking() {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        ...
        const booked = querySnapshot.docs.map(
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        alert(
          "Désolé, ce créneau vient d'être réservé. Veuillez en choisir un autre.",
        );
  ...
  const generateCalendarLinks = () => {
    ...
    const title = encodeURIComponent(
      `Lavage Auto - ${siteConfig.name} - ${formData.service}`,
    );
    const details = encodeURIComponent(
      `Lavage Automobile par ${siteConfig.name}.\nModèle: ${formData.vehicleModel}\nLieu: Chez vous ou au centre (${siteConfig.contact.address.city})`,
    );
    const location = encodeURIComponent(siteConfig.contact.address.city);
    ...
    return (
      ...
      >
        ...
        <p className="text-zinc-400 mb-10 max-w-md text-balance font-light leading-relaxed">
          Merci pour votre réservation. Votre compte a été vérifié et nous vous
          contacterons très bientôt pour l'horaire exact.
        </p>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16 sm:py-24 relative">
      <Helmet>
        ...
        <meta
          name="description"
          content="Réservez en ligne votre lavage auto à Genève. Choisissez votre formule, votre date et votre horaire pour un nettoyage de voiture sur-mesure."
        />
      ...
      >
        ...
        <p className="text-zinc-400 mb-10 font-light text-lg">
          Rapide, simple & avec un sourire humain. Disponible 7J/7.
        </p>

        <form
          onSubmit={handleSubmit}
          className="space-y-8 bg-zinc-900/40 border border-zinc-800/50 rounded-3xl p-8 sm:p-10 shadow-2xl"
        >
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="rounded-xl h-12 bg-zinc-900/50 outline-none"
              />
            </div>
            <div className="space-y-2">
              <label
          ...
          <div className="space-y-2">
            ...
            >
              <option value="Citadine">
                {siteConfig.services.citadine.name} (
                {siteConfig.services.citadine.price}.-)
              </option>
              <option value="Moyenne">
                {siteConfig.services.moyenne.name} (
                {siteConfig.services.moyenne.price}.-)
              </option>
              <option value="Grande">
                {siteConfig.services.grande.name} (
                {siteConfig.services.grande.price}.-)
              </option>
            </select>
          </div>
              ...
              {formData.date &&
                ...
                bookedSlots.length >= ALL_SLOTS.length && (
                  <p className="text-sm text-red-400 mt-2">
                    Ce jour est complet. Veuillez choisir une autre date.
                  </p>
                )}
            </div>
          </div>
          ...
          <Button
            type="submit"
            className="w-full rounded-full h-14 text-lg mt-4"
            size="lg"
            disabled={loading}
          >
